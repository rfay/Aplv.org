...
function aplv2012d7_menu_local_task($link, $active = FALSE) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
 ...
 *   A descriptive title of the feed.
  */
function aplv2012d7_feed_icon($url, $title) {
 ...
 */
function aplv2012d7_preprocess_comment_wrapper(&$vars) {
 ...
 */
function aplv2012d7_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb art-postcontent">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function aplv2012d7_service_links_node_format($links) {
 ...
 * @ingroup themeable
 */
function aplv2012d7_button($element) {
...
*/
function aplv2012d7_img_assist_page($content, $attributes = NULL) {
 ...
 * @ingroup themeable
 */
function aplv2012d7_node_preview($node) {
